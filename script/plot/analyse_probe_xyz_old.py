...
def construct_omega_k_data(starttime,endtime,dx,dt,path,Cs):
	...
	print("FFTarray t = %f, x = %f"%(len(FFTarray[:,0]),len(FFTarray[0,:])))
	print("t = %f, x = %f"%(len(FFTtime),len(FFTx)))
	cs = ax.contourf(FFTx,FFTtime,abs(FFTarray[:Nt//2,:N//2]),1000) # ,locator=ticker.LogLocator()
...
def construct_omega_k(array):
	...
	for filter_k in range(N): # every k
		...
		for j in range(N/16): # length of box (x dim)
...
def construct_time_x_data(starttime,endtime,dx,dt,path,Cs):
	...
	kx     = (-nx/2 + np.arange(0,nx+1))*2*np.pi/(xmax)
	ky     = (-nt/2 + np.arange(0,nt+1))*2*np.pi/(ymax)
	#print(kx)
...
def find_omega_k(starttime,endtime,path,dt,dx):
	...
	Csa = np.sqrt((gamma_i*T_i+gamma_e*T_e)/M_i)
	Vd = E0/B0
	Cs = Vd/(1+Psi)

	#construct_omega_k_data(starttime,endtime,dx,dt,path,Cs)

	construct_time_x_data(starttime,endtime,dx,dt,path,Cs)


if __name__ == "__main__":
	find_omega_k(500,15000,path="",dt = 3e-6,dx=0.04)
