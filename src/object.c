...
void oFillLookupTables(Object *obj, const MpiInfo *mpiInfo) {
    ...
    //msg(STATUS,"obj->domain->sizeProd[obj->domain->rank] = %i",(obj->domain->sizeProd[obj->domain->rank]));
    ...
    for (long int i=0; i<obj->domain->sizeProd[obj->domain->rank]; i++) {
        if (obj->domain->val[i]>0.5 && !isGhostNode(obj->domain, i)){
            ...
            //msg(STATUS,"added %li to lookupInterior's index %li ",i,((index[(int)(obj->domain->val[i]+0.5)-1])) );
        }
    }
