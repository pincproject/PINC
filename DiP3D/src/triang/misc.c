 /*modified read_segments function*/
     ...
     int *genus;
{
  ...
  while(fscanf(infile, "%lf %lf %d %lf %lf %lf %lf %lf", &temp1, &temp2, &ncontours, &temp1,&temp1,&temp1,&temp1,&temp1 ) < 8)
    {    fscanf(infile, "%s", temp);}
	  fscanf(infile, "%s", temp); /*dust type*/
	  fscanf(infile, "%lf", &temp1);
	
  //printf("%E\t%E\t%d\n", temp1, temp2, ncontours);
  if (ncontours <= 0)
    return -1;
