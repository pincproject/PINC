...
#include "../const.h"
#include <sys/time.h>
/*Modified: main changed into d_polygon and modified*/
...
void d_polygon(int arc, char *arv[])
{
  ...
  for(j=1; j<arc; j++) /* for each dust grain do*/
    {  
      if ((arc < 2) || ((n = read_segments(arv[j], &genus)) < 0))
	{
	  fprintf(stderr, "warining: no dust grains!");
	}

      initialise(n);
      construct_trapezoids(n);
      nmonpoly = monotonate_trapezoids(n);
      //  printf("In dpolygon nmonpoly %d\n", nmonpoly);
     ntriangles = triangulate_monotone_polygons(n, nmonpoly, op);
     //   printf("In dpolygon\n");
 
   if(ntriangles!=nooftriangles[j-1])
	{printf("SOMETHING WRONG IN D_POLYGON!!!: %d %d\n", ntriangles, nooftriangles[j-1]); getchar();}
      /*now create a table for triangles*/   
      for (i = 0; i < ntriangles; i++)
	{
	  printf("TRIANGULATE: Dust %d trianle #%d: (%d,%d,%d)\n", j,i,op[i][0],op[i][1],op[i][2]);

	  /*note that in this algorithm vertices start from 1*/
	  /*it is only the index of vertex -> not the coordinate*/
	  dtrian[j-1][i].pt1=op[i][0]-1;
	  dtrian[j-1][i].pt2=op[i][1]-1;
	  dtrian[j-1][i].pt3=op[i][2]-1;
	}
    }
}
