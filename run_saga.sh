...
# Job name:
#SBATCH --job-name=MMO_noBooms_PHTemp_3EV
#
# Project:
...
#
# Outfile
#SBATCH --output=MMO_noBooms_PHTemp_3EV.out
...
cp $SUBMITDIR/MMOaphelion.ini $SCRATCH/$SLURM_JOB_ID
##cp $SUBMITDIR/decaetall.ini $SCRATCH/$SLURM_JOB_ID
mkdir $SCRATCH/$SLURM_JOB_ID/data
cp $SUBMITDIR/data/object.grid.h5 $SCRATCH/$SLURM_JOB_ID/data
